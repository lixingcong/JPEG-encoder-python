...
//文件功能：打印当前jpg文件的所有marker信息，由lixingcong修改添加多个输入文件支持
//原文件链接：
//https://github.com/andybusse/uavcamera/blob/master/ExtractorExecutable/jpegExtract.c
...
//----Provides all header titles for future development----
...
int main(int argc, char *argv[])
{
	...
	for (i = 1; i < argc; i++) {
		...
		if (in != NULL) {
			...
			fprintf(stdout, "#####################\nFileName:%s\n", argv[i]);
			operation_jpg();
			fprintf(stdout, "printf End of FileName:%s\n\n", argv[i]);
			if (in != stdin)
				fclose(in);
		} else {
...
uint8_t JPEGMethod(uint32_t JPEGCommand)
{
	...
	case DHT:
		...
		// INCOMPLETE: ONLY READS ONE QT PER DQT WHEN MULTIPLE
		// QT'S CAN OCCUR
	case DQT:
